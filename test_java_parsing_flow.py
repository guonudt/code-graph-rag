...
def test_java_parsing_without_querycursor():
    ...
    try:
        ...
        try:
            ...
            print(
                f"✅ 数据库连接成功，当前节点数: {result[0]['total'] if result else 0}"
            )
                ...
                try:
                    ...
                    if len(parts) >= 2:
                        ...
                        if import_stored <= 5:  # 只显示前5个
                            print(
                                f"  ✅ Import {import_stored}: {package_name}.{class_name}"
                            )
            ...
            if node.type in ["method_declaration", "constructor_declaration"]:
                ...
                try:
                    ...
                    if method_stored <= 10:  # 只显示前10个
                        print(
                            f"  ✅ 方法 {method_stored}: {method_name}() -> {return_type}"
                        )
        ...
        # 检查Package节点
        packages = ingestor.fetch_all(
            "MATCH (p:Package) RETURN p.name as name ORDER BY p.name"
        )
        print(f"  📦 Package节点: {len(packages)}个")

        # 检查Class节点
        classes = ingestor.fetch_all(
            "MATCH (c:Class) RETURN c.name as name, c.qualified_name as qualified_name ORDER BY c.name"
        )
        ...
        methods = ingestor.fetch_all(
            "MATCH (m:Method) RETURN m.name as name, m.return_type as return_type ORDER BY m.name"
        )
