...
def test_java_parsing_simulation():
    """测试Java解析逻辑，模拟数据库存储"""
    ...
    try:
            ...
            if node.type in ["method_declaration", "constructor_declaration"]:
                ...
                try:
                    ...
                    if method_count <= 10:  # 只显示前10个
                        print(f"  ✅ 方法 {method_count}: {method_name}({param_count}参数) -> {return_type}")
        ...
        if stored_data['classes']:
            ...
            print(f"  类示例: {cls['name']} (父类: {cls['superclass']}, 接口: {cls['interfaces']})")
        ...
        # 总结
        total_elements = len(stored_data['imports']) + len(stored_data['classes']) + len(stored_data['methods'])
        print(f"\n📈 总结:")
        ...
        if total_elements > 0:
            print(f"  ✅ Java解析逻辑工作正常！")
        else:
            print(f"  ❌ Java解析逻辑存在问题")
