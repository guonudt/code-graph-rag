...
def test_java_parsing_tools():
    ...
    try:
        # 导入我们的解析工具
        from codebase_rag.parsers.java_utils import extract_java_import_path, extract_java_class_info
        ...
        processor = DefinitionProcessor(ingestor, type_inference)
        ...
        def test_methods(node):
            ...
            if node.type in ["method_declaration", "constructor_declaration"]:
                ...
                try:
                    ...
                    # 测试方法类型推断
                    method_type = type_inference.infer_method_type(node, "com.taobao.uic.context.UicCommonContext")
        ...
        def test_calls(node):
            ...
            if node.type == "method_invocation":
                ...
                try:
                    ...
                    # 测试方法调用类型推断
                    call_type = type_inference.infer_method_call_type(node, "com.taobao.uic.context.UicCommonContext")
                    if call_count <= 10:  # 只显示前10个
                        print(f"  ✅ 调用 {call_count}: {object_name}.{method_name}() -> {call_type}")
                except Exception as e:
        ...
        if import_errors == 0 and class_errors == 0 and method_errors == 0 and call_errors == 0:
            print(f"\n✅ 所有解析工具工作正常！")
        else:
            print(f"\n❌ 发现解析问题，需要进一步调试")
